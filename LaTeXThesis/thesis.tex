...
\begin{document}
%todo: Jedes Chapter außer Einführung und Fazit benötigen ein Intro.
...
%todo: Anforderung über Vertraulichkeit: 
%Verschlüssung benutzen, keine RCE.
%In die eval kann dann, dass die architektur sicher ist, weil mqtt encrypted und api keine rce zulässt.

%todo: Wartbarkeit mit Logging verknüpfen.
%todo: Portabilität herausstellen. Soll auf Windows und Unixlike systemen gehen und mit unterschiedlichen Programmiersprachen.
%todo: Alle diese Anforderungen in der Eval aufgreifen.
Die Smartbit-Lösung muss die in den Aufgabenstellungen beschrieben Schnittstellen anbieten.
Um eine benutzerfreundliche Lösung umzusetzen, werden Anforderungen aus den Aufgabenstellungen abgeleitet, die Softwarequalität erhöhen.
Orientiert wird sich dafür an der ISO/IEC 25010:2011, die Software-Qualitätsmerkmale definiert \cite{software_quali}.
...
Latenzen werden durch die Round-Trip-Time (\acrshort{rtt}) gemessen.
...
Kollisions-Präventionsprinzipien wie \acrshort{csmaca} (Carrier Sense Multiple Access/Collision Avoidance) verhindern bei WLAN-Verbindungen zwar, das Geräte gleichzeitig senden, erhöhen dadurch allerdings die Latenzen.
Die Latenzen sind ebenfalls abhängig vom Standard des mobilen Netzwerks.
Bei WLAN 802.11b beträgt die Latenz ca. 10 ms, kann jedoch beispielsweise bei \acrshort{umts}-Verbindungen (Universal Mobile Telecommunications System) auf 300 ms bis 400 ms ansteigen \cite{network_latencies}.
...
%todo: das hier ist eher architektur
% Um diese Problematik zu mitigieren kann ein weiterer Ansatz ist, die Latenz vor der Programmiererin oder dem Programmierer durch die Verwendung eines Zwischenpuffers zu verschleiern.
...
%todo: cite mit anzahl der downloads / user
...
%todo: add mehrere sprachen
% warum gerade java, python und c
% vorteil: weit verbreitet und plattformübergreind.
%allerdings muss dass in die arch.
