...
\end{itemize}
Damit Sensoranfragen schnell beantwortet werden werden die aktuellen Sensorworte laufend vom Smartphone an den Server übermittelt.
...
\\
Vier Threads koordinieren die Abarbeitung von Anfragen.
Um Anfragen austauschen zu können kommen queues der Python-Implementierung cpython \cite{python_queue} zum Einsatz.
Sie erlauben ein threadsicheres Einsetzen und Entfernen von Nachrichten.
\\
Damit die Sensorwerte vorgehalten werden können muss ein Thread kontinuierlich auf Sensoraktualisierungsanfragen des Smartphones per MQTT hören.
Dies ist so
