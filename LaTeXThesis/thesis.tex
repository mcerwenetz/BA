...
Auf Mikrocontroller-Schaltungen basierende Projekte bieten durch angeschlossene Sensoren ein ideale Interaktivität, benötigen allerdings auch zusätzliches Fachwissen und müssen zusätzlich beschafft werden.
Gegenüber ihnen bietet der Einsatz von Smartphones für interaktive Aufgaben Vorteile gegenüber Mikrocontroller-Schaltungen.
...
Die Evaluation der Lösung erfolgt durch eine Demonstration einer Beispielaufgabe und einer Messung für drei unterschiedliche Nutzungsszenarien.
...
Bei Aufrufen auftretende Latenzen zwischen Smartphone und Programm beeinträchtigen nicht die Nutzung.
\end{abstract}
...
Der Fokus liegt auf den Anforderungen berücksichtigenden Designentscheidungen während der Implementierung der Lösung und den zur interkomponellen Kommunikation verwendeten Protokollen.
Des Weiteren wird das Verhalten der Lösung exemplarisch anhand des Ablaufs eines Funktionsaufrufs spezifiziert.
...
% Die Reduzierung der Latenzzeiten hat eine höhere Priorität, als eine Übertragungssicherheit. 
...
%todo: Keine Einbindung?
%todo: Wo stehen die Funktionen? Welche gibt es überhaupt. Beispiele!
...
In der Datei \textit{protocol.json} werden neben den Schlüsselwörtern und Abkürzungen für Sensortypen und Ausgabekommandos auch Vorlagen für Nachrichtenformate aufgeführt.
...
Android unterbindet UI-Änderungen aus Nicht-UI-Threads.
...
\end{figure}
Der MQTT-Service baut eine Verbindung zu, einem in Datei \textit{config.json} definierten, MQTT-Broker auf und abonniert ein, ebenfalls in der Konfigurationsdatei angegebenes, Topic.
...
% Todo: JSON wär auch gut in der Kontrollanwendung
...
Häufig versandte, nicht Ausgaben betreffende, Nachrichtentypen wie \texttt{sensor\_requests}, \texttt{sensor\_responses} und \texttt{update\_requests} werden nicht geloggt.
...
% todo: Auf jeden Fall in Programmierumgebung noch n Kapitel zu JSON Parsern
...
%todo: 4-5 Sätze damit 1,5 Seiten voll sind
