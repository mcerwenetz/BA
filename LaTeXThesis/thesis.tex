...
\label{fig:design}
\end{figure}
Um mit dem Smartphone zu interargieren bietet die Programmierumgebung eine programmiersprachenunabhängige Bibliothek mit Schnittstellen in Form von Stub-Funktionen, die die API der Kontrollanwendung nutzen.
Die Bibliothek kann in bestehenden Programmcode zum Zweck der Nutzung eingebunden werden.
Zur Unterstützung der Verständlichkeit für angehende Programmierer wurde bei der Entwicklung auf die Implementierung von Funktionen mit asynchronen Rückgabewerten verzichtet.
...
Die Nachricht wird zum Beispiel bei einer Ausgabeanfrage per MQTT an das Smartphone übertragen.
Die Kontrollanwendung speichert zudem Sensormesswerte unpersistent in einem assoziativen Feld mit Key-Value-Einträgen um sie gegenüber der Bibliothek zu cachen.
Diese werden Messwerte von der Smartphone-App in regelmäßigen Abständen an die Kontrollanwendung übermittelt.
Bei Eingang einer Sensorwert-Anfrage seitens der Bibliothek sendet die Kontrollanwendung den zuletzt von der Smartphone-App übermittelten Wert an die Bibliothek zurück.
...
Die im JSON Format gespeicherten Nachrichten-Vorlagen sind in einer Datei gespeichert.
Sie werden in der Bibliothek, dem Kontrollprogramm und in der Smartphone-App eingelesen.
So sind die Nachrichten in jeder Komponente gleich.
Es gibt unterschiedliche Nachrichtenformate, welche in Tabelle \ref{tab:message_types} aufgeführt sind werden für verschiedene Zwecke eingesetzt.
...
Eine Liste aller Nachrichtenformate, inklusive ihrer Felder, ist im Anhang aufgeführt.
\begin{table}[htbp]
  \centering
...
Nach Eingang ermittelt das Kontrollprogramm durch Angabe des den gewünschten Sensortyps im Feld \texttt{sensor\_type} den gespeicherten Sensormesswert.
...
Der Fokus liegt auf der internen Struktur der jeweiligen Komponenten ihrer jeweiligen Schnittstellen.
...
Die Anwendung initiiert zum Start die Sensormessprozesse und sendet unter Verwendung von SensorEventListenern die Sensormessdaten in periodischen Zeitabständen an die Kontrollanwendung.
...
Die RootActivity und Service tauschen jeweils ihre Objekt-Referenzen aus.
